...
# -*- coding: utf-8 -*-
"""
NBA Analyzer - Module Backtest
Enregistre la route /api/backtest sur l'app Flask principale.
Usage: from nba_analyzer_backtest import register_backtest_routes
...
def _simulate_line(historical_values):
    """Simule une ligne bookmaker via moyenne ponderee recente."""
...
def _line_value(historical_series, line):
    """Ecart entre forme recente et ligne — mesure d'inefficacite."""
...
def _run_walkforward(player_name, stat_type, season, min_edge, stake, collector):
    ...
    for i in range(MIN_TRAIN, len(df)):
        ...
        # Filtre outlier dans le backtest aussi: si le match courant < 20 min, skip
        ...
        # Memes filtres que l'app live
        if rec == 'SKIP' or edge < min_edge or train_r2 < 0.05 or lv < 0.3:
            continue
    ...
    if not bets:
        return {
            'status':            'NO_BETS',
            'message':           f'0 pari qualifie (edge>={min_edge}%, line_value>=0.3)',
...
def register_backtest_routes(app, collector):
    """
    Enregistre /api/backtest sur l'app Flask passee en parametre.
    Appeler apres la creation de l'app dans nba_analyzer_improved.py
    """
